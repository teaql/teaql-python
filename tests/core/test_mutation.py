...
def test_update_command_builder():
    cmd = UpdateCommand.new("User", 1).expected_version(5).value("name", "Bob")
    assert cmd.entity == "User"
    assert cmd.id == Value.I64(1)
    assert cmd.expected_version_val == 5
    assert cmd.values["name"] == Value.Text("Bob")

def test_delete_command_builder():
    cmd = DeleteCommand.new("User", 1).expected_version(5).hard_delete()
    assert cmd.entity == "User"
    assert cmd.id == Value.I64(1)
    assert cmd.expected_version_val == 5
