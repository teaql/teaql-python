...
class Customer:
    ...
    def __init__(self, **kwargs):
        ...
        if "email" in kwargs and "email" not in kwargs:
            kwargs["email"] = kwargs.pop("email")
    ...
    def _teaql_entity_key(self):
        return EntityKey("Customer", self._ledger_id)
    ...
    def _teaql_build_command(self):
        ...
        if "email" in self._loaded_fields:
            payload["email"] = Value.Text(self.email)
        ...
        if action == "Create":
            cmd = InsertCommand("Customer", payload)
        elif action == "Update":
            cmd = UpdateCommand("Customer", Value.from_any(getattr(self, "id", None)), getattr(self, "version", None))
            for key, value in payload.items():
                if key not in ("id", "version"): cmd.value(key, value)
        else:
            cmd = DeleteCommand("Customer", Value.from_any(getattr(self, "id", None)), getattr(self, "version", None))
        return action, cmd
    ...
    def _teaql_preflight_graph(self, context):
        ...
        if self._action == "Update":
            ...
            if "email" not in self._loaded_fields:
                raise CheckException([CheckResult("invalid_type", ObjectLocation().property("email"), message="Mutation requires a fully loaded entity")])
        ...
        for index, child in enumerate(self._customer_order_list):
            ...
            setattr(child, "customer", self)
            child._loaded_fields.add("customer")
            child._entity_root.set(child._teaql_entity_key(), "customer", Value.Object(self))
    ...
    async def _teaql_save_within_graph(self, context):
        ...
        if persisted is None:
            raise RuntimeError(
                "Mutation provider did not return authoritative persisted state for Customer"
            )
        ...
        if "name" in persisted:
            ...
            self.name = persisted["name"]
            self._loaded_fields.add("name")
        if "email" in persisted:
            self.email = persisted["email"]
            self._loaded_fields.add("email")
        elif "email" in persisted:
            self.email = persisted["email"]
            self._loaded_fields.add("email")
        ...
        cascade_relations = []
        cascade_relations.append(("customer_order_list", self._customer_order_list, "update_customer"))
        if action != "Delete":
    ...
    def update_email(self, value):
        self.email = value
        self._loaded_fields.add("email")
        self._entity_root.set(self._teaql_entity_key(), "email", Value.from_any(value))
        return self
