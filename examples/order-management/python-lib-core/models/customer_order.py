...
class CustomerOrder:
    ...
    async def save(self, ctx):
        ...
        if getattr(self, "orderNumber", None) is not None:
            payload["order_number"] = Value.Text(self.orderNumber)
        if getattr(self, "orderDate", None) is not None:
            payload["order_date"] = Value.Date(self.orderDate)
        if getattr(self, "totalAmount", None) is not None:
            payload["total_amount"] = Value.Object(self.totalAmount)
        ...
        cascade_relations = []
        cascade_relations.append((self._order_line_list, "update_customer_order"))
        if action != "Delete":
