...
class OrderStatus:
    ...
    def _teaql_entity_key(self):
        return EntityKey("OrderStatus", self._ledger_id)
    ...
    def _teaql_build_command(self):
        ...
        if action == "Create":
            ...
            cmd = UpdateCommand("OrderStatus", Value.from_any(getattr(self, "id", None)), getattr(self, "version", None))
            for key, value in payload.items():
                if key not in ("id", "version"): cmd.value(key, value)
        else:
            cmd = DeleteCommand("OrderStatus", Value.from_any(getattr(self, "id", None)), getattr(self, "version", None))
        return action, cmd
    ...
    async def _teaql_save_within_graph(self, context):
        ...
        if persisted is None:
            raise RuntimeError(
                "Mutation provider did not return authoritative persisted state for OrderStatus"
            )
        ...
        cascade_relations = []
        cascade_relations.append(("customer_order_list", self._customer_order_list, "update_status"))
        if action != "Delete":
