...
class OrderSearchPreset:
    ...
    def _teaql_build_command(self):
        ...
        if action == "Create":
            ...
            cmd = UpdateCommand("OrderSearchPreset", Value.from_any(getattr(self, "id", None)), getattr(self, "version", None))
            for key, value in payload.items():
                if key not in ("id", "version"): cmd.value(key, value)
        else:
            cmd = DeleteCommand("OrderSearchPreset", Value.from_any(getattr(self, "id", None)), getattr(self, "version", None))
        return action, cmd
