...
class PlatformExpression(EntityExpression):
    ...
    def school_type_list(self):
        path = self._path_for("school_type_list")
        if self._error is not None:
            return ListExpression([], self._root, path, SchoolTypeExpression, self._error)
        if self._value is None:
            return ListExpression([], self._root, path, SchoolTypeExpression)
        if "school_type_list" not in getattr(self._value, "_loaded_fields", set()):
            return ListExpression([], self._root, path, SchoolTypeExpression, self._not_loaded("school_type_list"))
        return ListExpression(getattr(self._value, "_school_type_list"), self._root, path, SchoolTypeExpression)
    def school_list(self):
        path = self._path_for("school_list")
        if self._error is not None:
            return ListExpression([], self._root, path, SchoolExpression, self._error)
        if self._value is None:
            return ListExpression([], self._root, path, SchoolExpression)
        if "school_list" not in getattr(self._value, "_loaded_fields", set()):
            return ListExpression([], self._root, path, SchoolExpression, self._not_loaded("school_list"))
        return ListExpression(getattr(self._value, "_school_list"), self._root, path, SchoolExpression)
    pass

class SchoolTypeExpression(EntityExpression):
    def id(self):
        return self._scalar("id")
    def name(self):
        return self._scalar("name")
...
class SchoolExpression(EntityExpression):
    def id(self):
        return self._scalar("id")
    def name(self):
        return self._scalar("name")
    def address(self):
        return self._scalar("address")
    def established_date(self):
        return self._scalar("establishedDate")
    def student_capacity(self):
        return self._scalar("studentCapacity")
    def active(self):
    ...
    def platform(self):
        return self._relation("platform", PlatformExpression)
    def school_type_id(self):
        return self._scalar("schoolType", relation_id=True)

    def school_type(self):
        return self._relation("schoolType", SchoolTypeExpression)
    pass
...
class E:
    ...
    def school_type(value):
        entity_id = getattr(value, "id", None)
        return SchoolTypeExpression(value, "SchoolType(id={})".format(entity_id))
    @staticmethod
    def school(value):
        entity_id = getattr(value, "id", None)
        return SchoolExpression(value, "School(id={})".format(entity_id))
    pass